...
void spirals( double x, double y, double length, double radius, double theta ); //prototype
...
{ 
  ...
  while(loop) // runs while loop is true menu(); // displays the menu to the user in the graphics window
  {
    if (gfx_event_waiting()) // the program to waits until something is pressed
    {
      command = gfx_wait();
      switch (command) 
      {
        ...
        case '8':  // spiral of spirals
          gfx_clear();
          spirals( 400, 400, 1, 0.9, 2*3.14159/3 );
          break;
...
void circularLace( double xc, double yc, double radius )
{
  gfx_color( 153, 51, 255 );
...
void snowflake( double x, double y, double radius, double theta )
{
  double x2, y2;
  for ( int i = 0; i < 5; i++ )
  {
...
void tree ( double x, double y, double length, double theta )
{
  double x1, y1;
...
void spirals( double x, double y, double length, double radius, double theta )
{
  gfx_color( 255, 204, 229 );
  ...
  // recursive calls 
  spirals( x, y, length, radius, theta );
}
